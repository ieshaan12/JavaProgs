...
// Uncomment the following lines when you execute the code for the first time to //create the database and comment it after the first time
	//try {
		...
		editbtn2.addActionListener(new ActionListener() {
			...
			st.executeUpdate("update CUSTOMER_INFO set CUSTOMER_NAME='"+value2+"', CUSTOMER_EMAIL='"+value3+"',CUSTOMER_COUNTRY='"+value4+"',CUSTOMER_GENDER='"+value5+"' where CUSTOMER_ID="+value1+"");
		...
		con = DriverManager.getConnection("jdbc:derby:cust;create=true;user=app;password=app");
			System.out.println("Successfully Connected to the database!");
			 ...
			 try {
				 ...
				 ResultSet results = statement.executeQuery("SELECT * FROM CUSTOMER_INFO");
